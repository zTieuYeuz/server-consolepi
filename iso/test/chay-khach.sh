...
# menu SAU moc nay (--since -30s tung bat nham lan tai cu -> bam phim qua som).
MOC=$(bash may-chu-iso.sh ssh "date +%s")
bash may-chu-iso.sh khach $FW start >/dev/null
for i in $(seq 1 60); do bash may-chu-iso.sh ssh "sudo journalctl -u console-pi-dashboard --since @$MOC --no-pager | grep -q menu.ipxe" && break; sleep 3; done
...
bash may-chu-iso.sh ssh "sudo journalctl -u console-pi-dashboard --since @$MOC --no-pager | grep pxeboot | awk '{print \$12}' | uniq -c"
